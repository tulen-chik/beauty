...
export default function ProfilePage() {
  ...
  const [modalCode] = useState("Qn4dh6JH7gs");
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-6 px-1 sm:px-4 w-full max-w-5xl mx-auto">
      {/* Верхний блок: аватар, имя, рейтинг */}
      <div className="relative mb-3 sm:mb-6 px-1 sm:px-4">
        {/* Блок пользователя с фоном */}
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 relative z-10 bg-black-01 rounded-t-2xl sm:rounded-t-[32px] rounded-b-2xl sm:rounded-b-[34px] px-2 sm:px-6 pt-3 pb-3 sm:pb-6">
          <div className="relative mb-2 sm:mb-0">
            <Image
              ...
              width={48}
              height={48}
              className="rounded-full object-cover border-4 border-black-02 w-12 h-12 sm:w-20 sm:h-20"
            />
            <span className="absolute bottom-1 right-1 w-3 h-3 bg-orange-500 rounded-full border-2 border-black-02" />
          </div>
          <div className="flex-1 w-full">
            <div className="flex sm:flex-row items-center gap-1 sm:gap-4">
              <span className="text-xl sm:text-3xl font-bold text-white">{username}</span>
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-gray-400 text-sm sm:text-lg ml-2">0.0</span>
            </div>
          </div>
          <span className="ml-auto text-gray-400 hidden sm:block">{t('profile.thisIsYou')}</span>
        </div>
        {/* Оранжевая подложка чуть ниже блока пользователя */}
        <div
          className="absolute left-1 right-1 sm:left-4 sm:right-4 -bottom-[1px] h-[20px] sm:h-[34px] rounded-b-full pointer-events-none"
          style={{
      ...
      <div className="flex flex-col gap-1 sm:gap-3 px-1 sm:px-0">
        <button className="flex items-center justify-between bg-black-02 rounded-xl px-2 sm:px-4 py-2 sm:py-[15px] text-white text-sm sm:text-lg">
...
</svg></span> <span className="xs:inline">{t('profile.changePassword')}</span>
          </span>
...
</svg></span> <span className="xs:inline">{t('profile.changeWallet')}</span>
          </span>
          <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
<rect width="40" height="40" rx="12" fill="url(#paint0_linear_646_3047)"/>
<path d="M18 14.5L24 20.5L18 26.5" stroke="white" strokeWidth="2" strokeLinecap="round"/>
...
<stop offset="1" stopColor="#FF883D"/>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:gap-4">
        <button
          className="flex items-center gap-2 bg-black-02 rounded-xl px-3 py-3 w-full text-white"
      ...
      {/* Нижнее меню */}
      <div
        className="flex gap-1 w-full sm:gap-4 mt-3 pb-2 cursor-grab select-none"
        ref={dragScroll.ref}
      ...
      >
        {tabs.map(tab => (
          <button
            ...
            className={`
              flex-1 px-2 py-2 rounded-xl font-bold text-xs
            ...
            `}
          >
            {t(`profile.tabs.${tab.label}`)}
          </button>
        ))}
      </div>

      {/* Conditional content rendering */}
      <div className="mt-3 sm:mt-6 mb-[40px] flex-1">
        {renderContent()}
      </div>
    </div>
  );
