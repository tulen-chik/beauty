    ...
    <main className="flex-1 flex flex-col h-full bg-rose-50 animate-pulse">
      <header className="flex items-center gap-3 p-3 bg-white border-b border-gray-200">
        ...
        <div>
          ...
          <div className="h-3 w-24 bg-gray-200 rounded mt-1.5"></div>
        </div>
      </header>
...
//=========== Компонент: 1. Элемент в списке чатов (SalonChatItem) ===========//

function SalonChatItem({ chat, isActive, onClick }: { chat: Chat, isActive: boolean, onClick: () => void }) {
  ...
  return (
    ...
    >
      <InitialAvatar name={chat.customerName || ''} className="w-14 h-14 rounded-full flex-shrink-0 text-xl" />
            ...
            <div className="flex-grow min-w-0">
              {isLoadingDetails ? (
                ...
                <span className="text-xs bg-green-100 text-green-700 font-medium px-2 py-0.5 rounded-full truncate inline-block max-w-full">
                 ...
                 <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{t('chatByAppointment')}</span>
              ) : null}
            </div>
  ...
  if (!selectedChatId) {
    return (
      <main className="hidden md:flex flex-1 flex-col items-center justify-center bg-gray-50">
        ...
        <h2 className="text-xl text-gray-600">Выберите чат для ответа</h2>
      </main>
    );
  }
