...
import { PasswordResetConfirmFormData } from '@/lib/validations/auth';
...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.new_password.length < 8) {
      setPasswordError(t('minPasswordLength'));
      return;
    }
    ...
    return (
      <div className="bg-black-02 rounded-3xl shadow-xl p-10 w-full max-w-md flex flex-col gap-6 relative">
        <div className="flex flex-col gap-4">
          ...
          <h1 className="text-4xl font-bold text-white">{t('invalidResetLink')}</h1>
          <p className="text-gray text-base">{t('invalidResetLinkMessage')}</p>
    ...
    <div className="bg-black-02 rounded-3xl shadow-xl p-10 w-full max-w-md flex flex-col gap-6 relative">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-2 items-center">
          ...
          <h1 className="text-4xl font-bold text-white">{t('resetPassword')}</h1>
        </div>
        <p className="text-gray text-base">{t('newPassword')}</p>
      </div>
      
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        {(error || passwordError) && (
          <div>
            <p className="text-red-500 text-sm">{error?.message || passwordError}</p>
            {error?.validationErrors?.map((err, index) => (
              <p key={index} className="text-red-500 text-sm">{err.field}: {err.message}</p>
            ))}
            <button onClick={() => { clearError(); setPasswordError(''); }} className="text-red-500 text-sm">×</button>
          </div>
        )}
