    ...
    <main className="min-h-screen flex items-center justify-center bg-black-01 p-4">
      <div className="bg-black-02 rounded-3xl shadow-xl p-10 w-full max-w-md flex flex-col gap-6">
        <div className="flex flex-col gap-4">
          ...
          <h1 className="text-4xl font-bold text-white">{t('errorTitle')}</h1>
          <p className="text-gray text-base">{t('errorDescription')}</p>
        </div>

        <ErrorDisplay 
          error={{ 
            message: error.message || t('unknownError'),
            validationErrors: error.digest ? [{ field: 'digest', message: error.digest }] : undefined
          }} 
        />
        ...
        <div className="flex flex-col gap-4 mt-4">
          <button
            onClick={reset}
            className="w-full py-3 rounded-xl text-lg text-white bg-gradient-to-r from-[#FF4400] to-[#FF883D] hover:opacity-90 transition"
          >
            {t('tryAgain')}
          </button>
          <Link 
            href="/"
            className="w-full py-3 rounded-xl text-lg text-center text-white bg-black-03 hover:bg-black-04 transition"
          >
            {t('backToHome')}
          </Link>
        </div>
