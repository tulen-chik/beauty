...
export interface OfferCardProps {
  ...
  price: string;
  minViews: string;
  socials: ReadonlyArray<"instagram" | "tiktok">;
  chatOpen?: boolean;
  onChatToggle?: () => void;
}
...
export default function OfferCard({
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden shadow-xl bg-[#191919] flex flex-col p-0 min-h-[340px] border border-[#232323]">
      {/* Верхний баннер с изображением */}
      <div className="w-full flex items-center justify-center bg-[#191919] relative" style={{ minHeight: 180 }}>
        <Image 
          src={logo} 
          alt="logo" 
          width={180} 
        ...
        <div className="absolute top-2 right-2 z-10 sm:top-3 sm:right-3">
          <button className="bg-[#232323] rounded-xl p-1 shadow-md hover:bg-[#282828] transition sm:p-2">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17.93c-2.83.48-5.48-1.51-5.96-4.34-.09-.52.36-.93.88-.93.44 0 .81.32.88.75.34 2.03 2.36 3.36 4.39 3.02 1.7-.28 2.98-1.7 2.98-3.42 0-1.72-1.28-3.14-2.98-3.42-2.03-.34-4.05.99-4.39 3.02-.07.43-.44.75-.88.75-.52 0-.97-.41-.88-.93.48-2.83 3.13-4.82 5.96-4.34 2.83.48 4.82 3.13 4.34 5.96-.48 2.83-3.13 4.82-5.96 4.34z" fill="#fff"/></svg>
          </button>
        </div>
      </div>
      {/* Контент карточки */}
      <div className="flex flex-col gap-3 bg-black-02 rounded-b-2xl p-2 pt-0 sm:gap-4 sm:p-4 sm:pt-0">
        {/* 1. Название и автор */}
        <div className="flex flex-col gap-1 mt-2 sm:gap-2">
          <div className="text-white text-base font-bold leading-tight text-left sm:text-[17px]">{title}</div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative w-12 h-12 rounded-full overflow-hidden">
              <Image src={avatar} alt="avatar" fill className="object-cover" loading="lazy" sizes="48px" />
            </div>
            <span className="text-white font-semibold text-sm leading-tight mr-2 sm:text-base">{user}</span>
            <span className="text-xs text-gray-400">до {deadline}</span>
            <div className="ml-auto flex items-center gap-1 bg-[#232323] rounded-lg px-2 py-0.5">
              <span className="text-orange-400 text-xs font-bold sm:text-sm">★</span>
              <span className="text-white text-xs font-semibold ml-1 sm:text-sm">{rating.toFixed(1)}</span>
            </div>
          </div>
        </div>
        {/* 2. Ракета и статистика */}
        <div className="flex items-stretch gap-2 sm:flex-row sm:gap-4 sm:items-center">
          {/* Ракета */}
          <div className="flex flex-col items-center justify-center min-w-[50px] max-w-[50px] sm:min-w-[70px] sm:max-w-[70px] mx-auto sm:mx-0">
            {getRocketSVG(percent)}
          </div>
          {/* Статистика */}
          <div className="flex-1 flex flex-col gap-1 sm:gap-2">
            <span className="text-white font-extrabold text-xl drop-shadow mb-1 sm:text-2xl" style={{letterSpacing: '-2px'}}>{percent}%</span>
            <div className="flex items-center bg-[#232323] rounded-xl px-3 py-1 sm:px-4 sm:py-2">
              <span className="text-white text-[13px] font-bold sm:text-[15px]">{current}</span>
              <span className="text-gray-400 text-[13px] font-semibold ml-2 sm:text-[15px]">из {total}</span>
            </div>
            <div className="flex items-center bg-[#232323] rounded-xl px-3 py-1 sm:px-4 sm:py-2">
              <span className="text-orange-400 text-[13px] font-bold sm:text-[15px]">{price} <span className="text-white">/ 1K</span></span>
              <span className="text-gray-400 text-[13px] font-semibold ml-2 sm:text-[15px]">просмотров</span>
            </div>
            <div className="flex items-center bg-[#232323] rounded-xl px-3 py-1 sm:px-4 sm:py-2">
              <span className="text-gray-400 text-[13px] font-semibold sm:text-[15px]">Выплаты с </span>
              <span className="text-orange-400 text-[13px] font-bold ml-1 sm:text-[15px]">{minViews}</span>
              <span className="text-gray-400 text-[13px] font-semibold ml-1 sm:text-[15px]">просмотров</span>
            </div>
          </div>
        </div>
        {/* 3. Кнопка чата и соцсети */}
        <div className="flex flex-col gap-2 mt-2 sm:flex-row sm:items-center sm:gap-2">
          <div className="flex gap-2 justify-center sm:justify-start">
            {socials.map((s) => <span key={s}><SocialIcon type={s} /></span>)}
          </div>
          <button
            className="w-full flex items-center justify-center gap-2 bg-[#232323] hover:bg-gradient-to-r hover:from-[#FF4400] hover:to-[#FF883D] text-white font-semibold px-4 py-2 rounded-xl transition text-base shadow-md sm:w-auto sm:ml-auto"
            onClick={() => setShowModal(true)}
            style={{ minWidth: 0 }}
          >
            <span className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-[#FF4400] to-[#FF883D]">
              <svg width="22" height="22" fill="none" viewBox="0 0 22 22"><circle cx="11" cy="11" r="11" fill="url(#paint0_linear)"/><path d="M6.5 8.5C6.5 7.11929 7.61929 6 9 6H13C14.3807 6 15.5 7.11929 15.5 8.5V13.5C15.5 14.8807 14.3807 16 13 16H9C7.61929 16 6.5 14.8807 6.5 13.5V8.5Z" stroke="white" strokeWidth="1.5"/><circle cx="11" cy="11" r="2" fill="white"/></svg>
            </span>
            <span className="font-semibold text-white text-base whitespace-nowrap">{chatOpen ? 'Скрыть чат' : 'Раскрыть чат'}</span>
          </button>
        </div>
      </div>
      {showModal && (
        <ModalOffer open={showModal} offer={{
          logo,
          background,
          avatar,
          user,
